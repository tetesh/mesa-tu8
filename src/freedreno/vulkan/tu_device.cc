                      ...
                      struct vk_device_extension_table *ext)
{
   ...
   *ext = (struct vk_device_extension_table){ .table = {
   ...
   // ANDROID: не завязываемся на ugralloc(), на KGSL всегда объявляем AHB/ANB
#if DETECT_OS_ANDROID
   if (device->instance && device->instance->knl &&
       strcmp(device->instance->knl->name, "kgsl") == 0) {
      ext->ANDROID_external_memory_android_hardware_buffer = true;
      ext->ANDROID_native_buffer = true;
      // Полезно для старых DXVK: внешние форматы с резолвом
                        ...
                        struct tu_instance *instance)
{
   ...
   case 7:
      ...
      break;
   case 8:
      // Раньше здесь насильно ставились TU_DEBUG_NOLRZ/TU_DEBUG_FLUSHALL.
      // Убираем форс: оставляем управление через переменную окружения TU_DEBUG.
      vk_device_dispatch_table_from_entrypoints(
         &dispatch_table, &tu_device_entrypoints_a8xx, false);
      break;
   default:
      unreachable("unknown fd_dev_gen");
      break;
   }
